...
public class AddFileDialog extends Dialog {
    ...
    private Logger logger = Logger.getLogger(AddFileDialog.class);
    ...
    public AddFileDialog(Shell parent, Controller controller, Engine engine, Folder folder) {
        super(parent, SWT.DIALOG_TRIM | SWT.APPLICATION_MODAL);
        this.controller = controller;
        setText(Labels.get("AddFileDialog.WindowTitle"));
        this.engine = engine;
        this.parent = folder;
    }
    ...
    private void createContents(final Shell shell) {
        ...
        label.setText(Labels.get("AddFileDialog.LabelName"));
        fd = new FormData();
        ...
        label = new Label(shell, SWT.NONE);
        label.setText(Labels.get("AddFileDialog.LabelData"));
        fd = new FormData();
        ...
        button.setText(Labels.get("AddFileDialog.ButtonBrowse"));
        fd = new FormData();
        ...
        button.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent arg) {
				...
				textName.setText(new File(filename).getName());
        ...
        button.setText(Labels.get("AddFileDialog.LabelPublished"));
        fd = new FormData();
        ...
        button = new Button(shell, SWT.PUSH);
        button.setText(Labels.get("AddFileDialog.ButtonClose"));
        fd = new FormData();
        ...
        button = new Button(shell, SWT.PUSH);
        button.setText(Labels.get("AddFileDialog.ButtonAdd"));
        fd = new FormData();
        fd.bottom = new FormAttachment(100, -10);
        ...
        button.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent arg) {
				...
				if (name.length() == 0) {
					UITool.warningBox(shell, Labels.get("AddFileDialog.NameRequired"));
					return;
				}
				if (name.lastIndexOf(".") < 0) {
					UITool.warningBox(shell, Labels.get("AddFileDialog.ExtensionRequired"));
					return;
				}
				if (findFileByName(name) != null) {
					UITool.warningBox(shell, Labels.get("AddFileDialog.NameExists"));
					return;
				}
				File file = new File(textData.getText());
				if (!file.exists()) {
					UITool.warningBox(shell, Labels.get("AddFileDialog.DataNotExists"));
					return;
				}
